...
![The Atrium desktop in dark mode](docs/desktop-dark.png)
...
When it works, `http://localhost:8787` shows the desktop with the eight seeded rooms, and `npm run status` prints the same state from the terminal.
...
| `PORT` | No | HTTP port for the server. Default `8787` |
...
| `ATRIUM_URL` | No | Base URL the CLI scripts talk to. Default `http://localhost:8787` |
...
Broken: `npm run dev` proxies `/api` to port 8788 while the server listens on 8787.
